---
title: "Ulbrich et al. Greenhouse exp Plant Responses Data Analyses"
...
dataSG_seed_surv <- read.csv(here::here("R_data","SG_surv_Seed_germ.csv"))
dataSG_trans_surv <- read.csv(here::here("R_data","SG_surv_transplant_replace.csv"))
data_SG_biomass <- read.csv(here::here("R_data","SG_TotalBiomass.csv"))
SG_trt <- read.csv(here::here("R_data","SG_trt_sheet.csv"))
SG_height_combin<- read.csv(here::here("R_data","SG_height_time_combine.csv"), header = T)
SG_planting_data<- read.csv(here::here("R_data","SG_planting_data.csv"), header = T)

SG_roottraits<- read.csv(here::here("R_data","SG_roottraits.csv"), header = T)
summary(SG_roottraits)

SG_inorg_N<- read.csv(here::here("R_data","SG_NO3NH4_ug_gdrysoil.csv"), header = T)
summary(SG_inorg_N)
SG_start_inorg_N<-read.csv(here::here("R_data","SG_start_NO3NH4_ug_gdrysoil.csv"), header = T)
...
simulateResiduals(fittedModel = day_t0_germ_model, plot = T)
```
...
df_anova_day_t0_germ_model=data.frame(row.names =c("Precipitation","Microbial treatment","Precip.*Mic.Treat."), "Df"=anova_day_t0_germ_model$NumDF,"Resid Df"=anova_day_t0_germ_model$DenDF,"F-value"=anova_day_t0_germ_model$`F value`,"P-value"=anova_day_t0_germ_model$`Pr(>F)`)
...
day_t0_soil_root_precip=summarise_at(dataSG_seed_1st_germ_trt_precip_soil_g, 
                                   "t0_germ", list(~n(),~mean(.),~sd(.),se=~sd(.)/sqrt(n())))
...
simulateResiduals(fittedModel = day_t0_germ_model_pres, plot = T)
```
...
df_anova_day_t0_germ_model_pres=data.frame(row.names =c("Precipitation","Microbial Presence","Precip.*Mic.Pres."), "Df"=anova_day_t0_germ_model_pres$NumDF,"Resid Df"=anova_day_t0_germ_model_pres$DenDF,"F-value"=anova_day_t0_germ_model_pres$`F value`,"P-value"=anova_day_t0_germ_model_pres$`Pr(>F)`)
kable(df_anova_day_t0_germ_model_pres,align = "c",digits = c(1,2,2,3), caption= "Days to germination")
```

## Microhabitat Days to first Germination
...
#0.01286
simulateResiduals(fittedModel = day_t0_germ_model_orig, plot = T)
```
Raw Statistical output
```{r}
...
df_anova_day_t0_germ_model_orig=data.frame(row.names =c("Precipitation","Microbial Source","Precip.*Mic.Source"),"Df"=anova_day_t0_germ_model_orig$NumDF,"Resid Df"=anova_day_t0_germ_model_orig$DenDF,"F-value"=anova_day_t0_germ_model_orig$`F value`,"P-value"=anova_day_t0_germ_model_orig$`Pr(>F)`)
...
#Average germination throughout the experiment
fin_dataSG_seed_surv_trt_tot_seedling%>% summarise_at("tot_num_germ", list(~n(),~mean(.),~sd(.),se=~sd(.)/sqrt(n())))
```

Diagnostic Graphs

```{r}
#Code for checking for over dispersion
...
simulateResiduals(fittedModel = seed_germ_model, plot = T)

```
Let's compare the Negative binomial to a binomial zero-inflated model
```{r}
library(glmmTMB)
seed_germ_model_ZI=glmmTMB(cbind(tot_num_germ, 10-tot_num_germ)~precip*soil_root+(1|block), ziformula=~1,family=binomial, data= fin_dataSG_seed_surv_trt_tot_seedling)
qqPlot(resid(seed_germ_model_ZI))
hist(resid(seed_germ_model_ZI))
shapiro.test(resid(seed_germ_model_ZI))
#p-value = 7.977e-05


simulateResiduals(fittedModel = seed_germ_model_ZI, plot = T)

#Residuals are slightly worse than negative binomial
```
...
df_Anova_seed_germ_model=data.frame(row.names =c("(Intercept)","Precipitation","Microbial treatment","Precip.*Mic.Treat."),"Df"=Anova_seed_germ_model$Df,"Chi-Sq"=Anova_seed_germ_model$Chisq,"P-value"=Anova_seed_germ_model$`Pr(>Chisq)`)
...
simulateResiduals(fittedModel = seed_germ_model_pres, plot = T)
```
Raw Statistical output
```{r}
...
df_Anova_seed_germ_model_pres=data.frame(row.names = c("(Intercept)","Precipitation","Microbial Presence","Precip.*Mic.Pres."),"Df"=Anova_seed_germ_model_pres$Df,"Chi-Sq"=Anova_seed_germ_model_pres$Chisq,"P-value"=Anova_seed_germ_model_pres$`Pr(>Chisq)`)
kable(df_Anova_seed_germ_model_pres,align = "c",digits = c(2,2,3), caption= "Number of seeds that germinated")
```

## Microhabitat Number of Seeds that Geminated 
...
simulateResiduals(fittedModel = seed_germ_model_org, plot = T)
```
Raw Statistical output
```{r}
...
df_Anova_seed_germ_model_org=data.frame(row.names =c("(Intercept)","Precipitation","Microbial Source","Precip.*Mic.Source"),"Df"=Anova_seed_germ_model_org$Df,"Chi-Sq"=Anova_seed_germ_model_org$Chisq,"P-value"=Anova_seed_germ_model_org$`Pr(>Chisq)`)
...
(pot_germination_rate_p=ggplot(germ_soil_root_precip, aes(x=precip,y=mean,fill=factor(soil_root,levels = treatment_order), ymin = mean-se, ymax= mean+se))+
    ...
    geom_errorbar(position=position_dodge(width=0.9), width=0.2, size=1)+
    scale_fill_manual(values = c( "white","lightgray", "darkgrey"),labels=c("Sterile","Bulk","Rhizo"), limits=treatment_order, name=NULL)+
    scale_x_discrete(labels=c("Ambient","Drought"), name=NULL)+ylab("Rate of germination\n(# of germinants)")+
...
#### Percentage Difference and magnitude live v sterile
```{r echo=FALSE}


germ_soil_root=fin_dataSG_seed_surv_trt_tot_seedling %>% group_by(soil_root)%>% summarise_at("tot_num_germ", list(~n(),~mean(.),~sd(.),se=~sd(.)/sqrt(n())))

#Percentage Difference
paste("Percentage difference =",((germ_soil_root[germ_soil_root$soil_root=="L.B",]$mean-germ_soil_root[germ_soil_root$soil_root=="S.B",]$mean)/germ_soil_root[germ_soil_root$soil_root=="S.B",]$mean)*100)

#number of germinants
paste("Number of germinants =",(germ_soil_root[germ_soil_root$soil_root=="L.B",]$mean-germ_soil_root[germ_soil_root$soil_root=="S.B",]$mean))

```

#### Percentage Difference live v sterile in Ambient
```{r echo=FALSE}
...
#Percentage Difference
paste("Percentage difference =",((germ_soil_root_precip[germ_soil_root_precip$precip=="A"&germ_soil_root_precip$soil_root=="L.B",]$mean-germ_soil_root_precip[germ_soil_root_precip$precip=="A"&germ_soil_root_precip$soil_root=="S.B",]$mean)/germ_soil_root_precip[germ_soil_root_precip$precip=="A"&germ_soil_root_precip$soil_root=="S.B",]$mean)*100)

#number of germinants
paste("Number of germinants =",germ_soil_root_precip[germ_soil_root_precip$precip=="A"&germ_soil_root_precip$soil_root=="L.B",]$mean-germ_soil_root_precip[germ_soil_root_precip$precip=="A"&germ_soil_root_precip$soil_root=="S.B",]$mean)

```

#### Percentage Difference Drought v Ambient
```{r echo=FALSE}
...
#Percentage Difference
paste("Percentage difference =",((germ_soil_precip[germ_soil_precip$precip=="D",]$mean-germ_soil_precip[germ_soil_precip$precip=="A",]$mean)/germ_soil_precip[germ_soil_precip$precip=="A",]$mean)*100)

#number of germinants
paste("Number of germinants =",germ_soil_precip[germ_soil_precip$precip=="D",]$mean-germ_soil_precip[germ_soil_precip$precip=="A",]$mean)
...
df_Anova_seed_final_plant_no_model=data.frame(row.names =c("(Intercept)","Precipitation","Microbial treatment","Precip.*Mic.Treat."),"Df"=Anova_seed_final_plant_no_model$Df,"Chi-Sq"=Anova_seed_final_plant_no_model$Chisq,"P-value"=Anova_seed_final_plant_no_model$`Pr(>Chisq)`)
...
df_Anova_seed_final_plant_no_model_pres=data.frame(row.names =c("(Intercept)","Precipitation","Microbial Presence","Precip.*Mic.Pres."),"Df"=Anova_seed_final_plant_no_model_pres$Df,"Chi-Sq"=Anova_seed_final_plant_no_model_pres$Chisq,"P-value"=Anova_seed_final_plant_no_model_pres$`Pr(>Chisq)`)
kable(df_Anova_seed_final_plant_no_model_pres,align = "c",digits = c(2,2,3), caption= "Presence Seedling pots")
```

### Microhabitat Cummulative germination and survival
Diagnostic Graphs
```{r}
...
df_Anova_seed_final_plant_no_model_orig=data.frame(row.names =c("(Intercept)","Precipitation","Microbial Source","Precip.*Mic.Source"),"Df"=Anova_seed_final_plant_no_model_orig$Df,"Chi-Sq"=Anova_seed_final_plant_no_model_orig$Chisq,"P-value"=Anova_seed_final_plant_no_model_orig$`Pr(>Chisq)`)
kable(df_Anova_seed_final_plant_no_model_orig,align = "c",digits = c(2,2,3), caption= "Microhabitat Seedling pots")
```
...
df_Anova_seed_surv_germ_model=data.frame(row.names =c("(Intercept)","Precipitation","Microbial treatment","Precip.*Mic.Treat."),"Df"=Anova_seed_surv_germ_model$Df,"Chi-Sq"=Anova_seed_surv_germ_model$Chisq,"P-value"=Anova_seed_surv_germ_model$`Pr(>Chisq)`)
...
#### Percent Difference live v sterile 
```{r echo=FALSE}


seed_surv_soil_root=fin_dataSG_biomass_seed_surv_trt_w_germ %>% group_by(soil_root) %>% summarise_at( 
                                   "surv_germ", list(~n(),~mean(.),~sd(.),se=~sd(.)/sqrt(n())))


paste("Percentage difference =",((seed_surv_soil_root[seed_surv_soil_root$soil_root=="L.B",]$mean-seed_surv_soil_root[seed_surv_soil_root$soil_root=="S.B",]$mean)/seed_surv_soil_root[seed_surv_soil_root$soil_root=="S.B",]$mean)*100)
...
paste("Percentage difference =",((seed_surv_soil_root_precip[seed_surv_soil_root_precip$precip=="A"&seed_surv_soil_root_precip$soil_root=="L.B",]$mean-seed_surv_soil_root_precip[seed_surv_soil_root_precip$precip=="A"&seed_surv_soil_root_precip$soil_root=="S.B",]$mean)/seed_surv_soil_root_precip[seed_surv_soil_root_precip$precip=="A"&seed_surv_soil_root_precip$soil_root=="S.B",]$mean)*100)
...
df_Anova_seed_surv_germ_model_pres=data.frame(row.names =c("(Intercept)","Precipitation","Microbial Presence","Precip.*Mic.Pres."),"Df"=Anova_seed_surv_germ_model_pres$Df,"Chi-Sq"=Anova_seed_surv_germ_model_pres$Chisq,"P-value"=Anova_seed_surv_germ_model_pres$`Pr(>Chisq)`)
kable(df_Anova_seed_surv_germ_model_pres,align = "c",digits = c(2,2,3), caption= "Survival given germination")
```

### Microhabitat survival given germination
Diagnostic Graphs
```{r}
...
df_Anova_seed_surv_germ_model_orig=data.frame(row.names =c("(Intercept)","Precipitation","Microbial Source","Precip.*Mic.Source"),"Df"=Anova_seed_surv_germ_model_orig$Df,"Chi-Sq"=Anova_seed_surv_germ_model_orig$Chisq,"P-value"=Anova_seed_surv_germ_model_orig$`Pr(>Chisq)`)
...
df_anova_seed_biomas_model=data.frame(row.names = c("Precipitation","Microbial treatment","Precip.*Mic.Treat."),"Df"=anova_seed_biomas_model$NumDF,"Resid Df"=anova_seed_biomas_model$DenDF,"F-value"=anova_seed_biomas_model$`F value`,"P-value"=anova_seed_biomas_model$`Pr(>F)`)
...
total_bio_soil_root_precip=summarise_at(fin_dataSG_biomass_seed_surv_trt_given_germ_soil_root_precip_g, 
                                       "total_biomass", list(~n(),~mean(.),~sd(.),se=~sd(.)/sqrt(n())))
...
df_anova_seed_biomas_model_pres=data.frame(row.names = c("Precipitation","Microbial Presence","Precip.*Mic.Pres."),"Df"=anova_seed_biomas_model_pres$NumDF,"Resid Df"=anova_seed_biomas_model_pres$DenDF,"F-value"=anova_seed_biomas_model_pres$`F value`,"P-value"=anova_seed_biomas_model_pres$`Pr(>F)`)
...
fin_dataSG_biomass_seed_surv_trt_given_germ_g=fin_dataSG_biomass_seed_surv_trt_given_germ %>% group_by(soil_root)
seed_biomass_soil_root=summarise_at(fin_dataSG_biomass_seed_surv_trt_given_germ_g, 
                                   "total_biomass", list(~n(),~mean(.),~sd(.),se=~sd(.)/sqrt(n())))


paste("Percentage difference =",((seed_biomass_soil_root[seed_biomass_soil_root$soil_root=="L.B",]$mean-seed_biomass_soil_root[seed_biomass_soil_root$soil_root=="S.B",]$mean)/seed_biomass_soil_root[seed_biomass_soil_root$soil_root=="S.B",]$mean)*100)

```

### Microhabitat total biomass of germinated
...
df_anova_seed_biomas_model_orig=data.frame(row.names = c("Precipitation","Microbial Source","Precip.*Mic.Source"),"Df"=anova_seed_biomas_model_orig$NumDF,"Resid Df"=anova_seed_biomas_model_orig$DenDF,"F-value"=anova_seed_biomas_model_orig$`F value`,"P-value"=anova_seed_biomas_model_orig$`Pr(>F)`)
...
df_anova_root_shoot_seed_model=data.frame(row.names = c("Precipitation","Microbial treatment","Precip.*Mic.Treat."),"Df"=anova_root_shoot_seed_model$NumDF,"Resid Df"=anova_root_shoot_seed_model$DenDF,"F-value"=anova_root_shoot_seed_model$`F value`,"P-value"=anova_root_shoot_seed_model$`Pr(>F)`)
...
root_shoot_soil_root_precip=summarise_at(fin_dataSG_biomass_seed_surv_trt_given_germ_soil_root_precip_g, 
                                   "root_shoot", list(~n(),~mean(.),~sd(.),se=~sd(.)/sqrt(n())))
...
df_anova_root_shoot_seed_model_pres=data.frame(row.names = c("Precipitation","Microbial Presence","Precip.*Mic.Pres."),"Df"=anova_root_shoot_seed_model_pres$NumDF,"Resid Df"=anova_root_shoot_seed_model_pres$DenDF,"F-value"=anova_root_shoot_seed_model_pres$`F value`,"P-value"=anova_root_shoot_seed_model_pres$`Pr(>F)`)
kable(df_anova_root_shoot_seed_model_pres,align = "c",digits = c(1,2,2,3), caption= "Root:Shoot given germination")
```
### Microhabitat Root:shoot ratio given germination
...
df_anova_root_shoot_seed_model_orig=data.frame(row.names = c("Precipitation","Microbial Source","Precip.*Mic.Source"),"Df"=anova_root_shoot_seed_model_orig$NumDF,"Resid Df"=anova_root_shoot_seed_model_orig$DenDF,"F-value"=anova_root_shoot_seed_model_orig$`F value`,"P-value"=anova_root_shoot_seed_model_orig$`Pr(>F)`)
...
```{r echo=FALSE, results='asis'}
anova_day_t0_germ_model_pres=anova(day_t0_germ_model_pres)
df_anova_day_t0_germ_model_pres=data.frame(row.names = c("Precipitation","Microbial treatment","Precip.*Mic.Treat."),"Df"=anova_day_t0_germ_model_pres$NumDF,"Resid Df"=anova_day_t0_germ_model_pres$DenDF,"F-value"=anova_day_t0_germ_model_pres$`F value`,"P-value"=anova_day_t0_germ_model_pres$`Pr(>F)`)
kable(df_anova_day_t0_germ_model_pres,align = "c",digits = c(1,2,2,3), caption= "Days to germination")
```
Graph for publication
```{r echo=FALSE}
...
soil_amm_seed_trt=summarise_at(SG_inorg_N_seed_trt_soil_root_precip_g, 
                               "ug_N_NH4_g_dry_soil_negto0", list(~n(),~mean(.),~sd(.),se=~sd(.)/sqrt(n())))
...
## SEEDLING PORTION OF THE EXPERIMENT BEGINING
Was there treatment effects on initial survival?
...
df_anova_SG_intial_height_model=data.frame(row.names = c("Precipitation","Microbial treatment","Precip.*Mic.Treat."),"Df"=anova_SG_intial_height_model$NumDF,"Resid Df"=anova_SG_intial_height_model$DenDF,"F-value"=anova_SG_intial_height_model$`F value`,"P-value"=anova_SG_intial_height_model$`Pr(>F)`)
...
df_anova_trans_biomas_model=data.frame(row.names = c("Precipitation","Microbial treatment","Precip.*Mic.Treat."),"Df"=anova_trans_biomas_model$NumDF,"Resid Df"=anova_trans_biomas_model$DenDF,"F-value"=anova_trans_biomas_model$`F value`,"P-value"=anova_trans_biomas_model$`Pr(>F)`)
...
trans_total_bio_soil_root_precip=summarise_at(data_SG_biomass_trans_surv_trt_soil_root_precip_g, 
                                        "total_biomass_0", list(~n(),~mean(.),~sd(.),se=~sd(.)/sqrt(n())))
...
paste("Percentage difference =",((trans_total_bio_precip[trans_total_bio_precip$precip=="D",]$mean-trans_total_bio_precip[trans_total_bio_precip$precip=="A",]$mean)/trans_total_bio_precip[trans_total_bio_precip$precip=="A",]$mean)*100)
...
paste("Percentage difference =",((trans_total_bio_soil_root_precip[trans_total_bio_soil_root_precip$precip=="A"&trans_total_bio_soil_root_precip$soil_root=="L.B",]$mean-trans_total_bio_soil_root_precip[trans_total_bio_soil_root_precip$precip=="A"&trans_total_bio_soil_root_precip$soil_root=="S.B",]$mean)/trans_total_bio_soil_root_precip[trans_total_bio_soil_root_precip$precip=="A"&trans_total_bio_soil_root_precip$soil_root=="S.B",]$mean)*100)
...
df_anova_trans_biomas_model_pres=data.frame(row.names = c("Precipitation","Microbial Presence","Precip.*Mic.Pres."),"Df"=anova_trans_biomas_model_pres$NumDF,"Resid Df"=anova_trans_biomas_model_pres$DenDF,"F-value"=anova_trans_biomas_model_pres$`F value`,"P-value"=anova_trans_biomas_model_pres$`Pr(>F)`)
...
data_SG_biomass_trans_surv_trt_soil_root_g=data_SG_biomass_trans_surv_trt %>% group_by(soil_root)
trans_total_bio_soil_root=summarise_at(data_SG_biomass_trans_surv_trt_soil_root_g, 
                                        "total_biomass_0", list(~n(),~mean(.),~sd(.),se=~sd(.)/sqrt(n())))


paste("Percentage difference =",((trans_total_bio_soil_root[trans_total_bio_soil_root$soil_root=="L.B",]$mean-trans_total_bio_soil_root[trans_total_bio_soil_root$soil_root=="S.B",]$mean)/trans_total_bio_soil_root[trans_total_bio_soil_root$soil_root=="S.B",]$mean)*100)

```


### Microhabitat at total biomass
...
df_anova_trans_biomas_model_orig=data.frame(row.names = c("Precipitation","Microbial Source","Precip.*Mic.Source"),"Df"=anova_trans_biomas_model_orig$NumDF,"Resid Df"=anova_trans_biomas_model_orig$DenDF,"F-value"=anova_trans_biomas_model_orig$`F value`,"P-value"=anova_trans_biomas_model_orig$`Pr(>F)`)
...
df_anova_root_shoot_model_na=data.frame(row.names = c("Precipitation","Microbial treatment","Precip.*Mic.Treat."),"Df"=anova_root_shoot_model_na$NumDF,"Resid Df"=anova_root_shoot_model_na$DenDF,"F-value"=anova_root_shoot_model_na$`F value`,"P-value"=anova_root_shoot_model_na$`Pr(>F)`)
...
df_anova_root_shoot_model_pres_na=data.frame(row.names = c("Precipitation","Microbial Presence","Precip.*Mic.Pres."),"Df"=anova_root_shoot_model_pres_na$NumDF,"Resid Df"=anova_root_shoot_model_pres_na$DenDF,"F-value"=anova_root_shoot_model_pres_na$`F value`,"P-value"=anova_root_shoot_model_pres_na$`Pr(>F)`)
kable(df_anova_root_shoot_model_pres_na,align = "c",digits = c(1,2,2,3), caption= "Root:Shoot Ratio")
```

### Microhabitat Root:shoot ratio
...
df_anova_root_shoot_model_orig_na=data.frame(row.names = c("Precipitation","Microbial Source","Precip.*Mic.Source"),"Df"=anova_root_shoot_model_orig_na$NumDF,"Resid Df"=anova_root_shoot_model_orig_na$DenDF,"F-value"=anova_root_shoot_model_orig_na$`F value`,"P-value"=anova_root_shoot_model_orig_na$`Pr(>F)`)
...
paste("Percentage difference =",((trans_survial_precip[trans_survial_precip$precip=="D",]$mean-trans_survial_precip[trans_survial_precip$precip=="A",]$mean)/trans_survial_precip[trans_survial_precip$precip=="A",]$mean)*100)
...
df_Anova_drought_surv_model=data.frame(row.names =c("(Intercept)","Microbial treatment"),"Df"=Anova_drought_surv_model$Df,"Chi-Sq"=Anova_drought_surv_model$Chisq,"P-value"=Anova_drought_surv_model$`Pr(>Chisq)`)
kable(df_Anova_drought_surv_model,align = "c",digits = c(2,2,3), caption= "Survival")
```

### Presence survival of the Seedling
...
df_Anova_drought_surv_model_pres=data.frame(row.names =c("(Intercept)","Microbial Presence"),"Df"=Anova_drought_surv_model_pres$Df,"Chi-Sq"=Anova_drought_surv_model_pres$Chisq,"P-value"=Anova_drought_surv_model_pres$`Pr(>Chisq)`)
kable(df_Anova_drought_surv_model_pres,align = "c",digits = c(2,2,3), caption= "Survival")
```


### Microhabitat survival of the Seedling####
...
df_Anova_drought_surv_model_orig=data.frame(row.names =c("(Intercept)","Microbial Source"),"Df"=Anova_drought_surv_model_orig$Df,"Chi-Sq"=Anova_drought_surv_model_orig$Chisq,"P-value"=Anova_drought_surv_model_orig$`Pr(>Chisq)`)
...
df_anova_SRL_trans_model=data.frame(row.names = c("Precipitation","Microbial treatment","Precip.*Mic.Treat."),"Df"=anova_SRL_trans_model$NumDF,"Resid Df"=anova_SRL_trans_model$DenDF,"F-value"=anova_SRL_trans_model$`F value`,"P-value"=anova_SRL_trans_model$`Pr(>F)`)
...
SRL_trans_soil_root_precip=summarise_at(SG_roottraits_trt_soil_root_precip_g, 
                                        "SRL_length_drymass", list(~n(),~mean(.),~sd(.),se=~sd(.)/sqrt(n())))
...
paste("Percentage difference =",((SRL_trans_precip[SRL_trans_precip$precip=="D",]$mean-SRL_trans_precip[SRL_trans_precip$precip=="A",]$mean)/SRL_trans_precip[SRL_trans_precip$precip=="A",]$mean)*100)
...
df_anova_SRL_trans_model_pres=data.frame(row.names = c("Precipitation","Microbial Presence","Precip.*Mic.Pres."),"Df"=anova_SRL_trans_model_pres$NumDF,"Resid Df"=anova_SRL_trans_model_pres$DenDF,"F-value"=anova_SRL_trans_model_pres$`F value`,"P-value"=anova_SRL_trans_model_pres$`Pr(>F)`)
kable(df_anova_SRL_trans_model_pres,align = "c",digits = c(1,2,2,3), caption= "SRL")
```

### Microhabitat specific root length
...
df_anova_SRL_trans_model_orig=data.frame(row.names = c("Precipitation","Microbial Source","Precip.*Mic.Source"),"Df"=anova_SRL_trans_model_orig$NumDF,"Resid Df"=anova_SRL_trans_model_orig$DenDF,"F-value"=anova_SRL_trans_model_orig$`F value`,"P-value"=anova_SRL_trans_model_orig$`Pr(>F)`)
...
RLD_trans_soil_root_precip=summarise_at(SG_roottraits_trt_soil_root_precip_g, 
                                                 "RLD_length_volume", list(~n(),~mean(.),~sd(.),se=~sd(.)/sqrt(n())))
...
df_anova_rhizosheath_model=data.frame(row.names = c("Precipitation","Microbial treatment","Precip.*Mic.Treat."),"Df"=anova_rhizosheath_model$NumDF,"Resid Df"=anova_rhizosheath_model$DenDF,"F-value"=anova_rhizosheath_model$`F value`,"P-value"=anova_rhizosheath_model$`Pr(>F)`)
...
RhizoSheath_trans_soil_root_precip=summarise_at(SG_roottraits_trt_soil_root_precip_g, 
                                        "RhizosheathSoil_DryRoots_g", list(~n(),~mean(.),~sd(.),se=~sd(.)/sqrt(n())))
...
df_anova_rhizosheath_model_pres=data.frame(row.names = c("Precipitation","Microbial Presence","Precip.*Mic.Pres."),"Df"=anova_rhizosheath_model_pres$NumDF,"Resid Df"=anova_rhizosheath_model_pres$DenDF,"F-value"=anova_rhizosheath_model_pres$`F value`,"P-value"=anova_rhizosheath_model_pres$`Pr(>F)`)
kable(df_anova_rhizosheath_model_pres,align = "c",digits = c(1,2,2,3), caption= "Rhizosheath presence")
```
### Microhabitat Rhizosheath 
...
df_anova_rhizosheath_model_orig=data.frame(row.names = c("Precipitation","Microbial Source","Precip.*Mic.Source"),"Df"=anova_rhizosheath_model_orig$NumDF,"Resid Df"=anova_rhizosheath_model_orig$DenDF,"F-value"=anova_rhizosheath_model_orig$`F value`,"P-value"=anova_rhizosheath_model_orig$`Pr(>F)`)
...
df_anova_SG_soil_nit_trans_model=data.frame(row.names = c("Precipitation","Microbial treatment","Precip.*Mic.Treat."),"Df"=anova_SG_soil_nit_trans_model$NumDF,"Resid Df"=anova_SG_soil_nit_trans_model$DenDF,"F-value"=anova_SG_soil_nit_trans_model$`F value`,"P-value"=anova_SG_soil_nit_trans_model$`Pr(>F)`)
...
df_anova_SG_soil_nit_trans_model_pres=data.frame(row.names = c("Precipitation","Microbial Presence","Precip.*Mic.Pres."),"Df"=anova_SG_soil_nit_trans_model_pres$NumDF,"Resid Df"=anova_SG_soil_nit_trans_model_pres$DenDF,"F-value"=anova_SG_soil_nit_trans_model_pres$`F value`,"P-value"=anova_SG_soil_nit_trans_model_pres$`Pr(>F)`)
kable(df_anova_SG_soil_nit_trans_model_pres,align = "c",digits = c(1,2,2,3), caption= "Soil nitrate")
```


## Microhabitat nitrate Seedling
...
df_anova_SG_soil_nit_trans_model_orig=data.frame(row.names = c("Precipitation","Microbial Source","Precip.*Mic.Source"),"Df"=anova_SG_soil_nit_trans_model_orig$NumDF,"Resid Df"=anova_SG_soil_nit_trans_model_orig$DenDF,"F-value"=anova_SG_soil_nit_trans_model_orig$`F value`,"P-value"=anova_SG_soil_nit_trans_model_orig$`Pr(>F)`)
...
df_anova_SG_soil_amm_trans_model=data.frame(row.names = c("Precipitation","Microbial treatment","Precip.*Mic.Treat."),"Df"=anova_SG_soil_amm_trans_model$NumDF,"Resid Df"=anova_SG_soil_amm_trans_model$DenDF,"F-value"=anova_SG_soil_amm_trans_model$`F value`,"P-value"=anova_SG_soil_amm_trans_model$`Pr(>F)`)
...
df_anova_SG_soil_amm_trans_model_pres=data.frame(row.names = c("Precipitation","Microbial Presence","Precip.*Mic.Pres."),"Df"=anova_SG_soil_amm_trans_model_pres$NumDF,"Resid Df"=anova_SG_soil_amm_trans_model_pres$DenDF,"F-value"=anova_SG_soil_amm_trans_model_pres$`F value`,"P-value"=anova_SG_soil_amm_trans_model_pres$`Pr(>F)`)
...
SG_ammonium_N_trt_LS_trt=SG_inorg_N_trt %>% group_by(soil_root) %>% summarise_at("ug_N_NH4_g_dry_soil_negto0", list(~n(),~mean(.),sd,se=~sd(.)/sqrt(n())))

paste("Percentage difference =",((SG_ammonium_N_trt_LS_trt[SG_ammonium_N_trt_LS_trt$soil_root=="S.B",]$mean-SG_ammonium_N_trt_LS_trt[SG_ammonium_N_trt_LS_trt$soil_root=="L.B",]$mean)/SG_ammonium_N_trt_LS_trt[SG_ammonium_N_trt_LS_trt$soil_root=="L.B",]$mean)*100)
```

### Microhabitat ammonium Seedling
...
df_anova_SG_soil_amm_trans_model_orig=data.frame(row.names = c("Precipitation","Microbial Source","Precip.*Mic.Source"),"Df"=anova_SG_soil_amm_trans_model_orig$NumDF,"Resid Df"=anova_SG_soil_amm_trans_model_orig$DenDF,"F-value"=anova_SG_soil_amm_trans_model_orig$`F value`,"P-value"=anova_SG_soil_amm_trans_model_orig$`Pr(>F)`)
...
df_anova_SG_soil_NITRATE_model=data.frame(row.names = c("Precipitation","Microbial treatment","Life-stage","Precip.*Mic.Treat.","Precip.*Life-stage","Mic.Treat.*Life-stage","Precip.* Mic.Treat. *Life-stage"),"Df"=anova_SG_soil_NITRATE_model$NumDF,"Resid Df"=anova_SG_soil_NITRATE_model$DenDF,"F-value"=anova_SG_soil_NITRATE_model$`F value`,"P-value"=anova_SG_soil_NITRATE_model$`Pr(>F)`)
...
paste("Percentage difference =",((SG_nitrate_N_trt_AC_trt[SG_nitrate_N_trt_AC_trt$precip=="D",]$mean-SG_nitrate_N_trt_AC_trt[SG_nitrate_N_trt_AC_trt$precip=="A",]$mean)/SG_nitrate_N_trt_AC_trt[SG_nitrate_N_trt_AC_trt$precip=="A",]$mean)*100)
...
df_anova_SG_soil_AMMON_model=data.frame(row.names = c("Precipitation","Microbial treatment","Life-stage","Precip.*Mic.Treat.","Precip.*Life-stage","Mic.Treat.*Life-stage","Precip.*Mic.Treat.*Life-stage"),"Df"=anova_SG_soil_AMMON_model$NumDF,"Resid Df"=anova_SG_soil_AMMON_model$DenDF,"F-value"=anova_SG_soil_AMMON_model$`F value`,"P-value"=anova_SG_soil_AMMON_model$`Pr(>F)`)
...
paste("Percentage difference =",((SG_ammonium_N_trt_AC_trt[SG_ammonium_N_trt_AC_trt$precip=="D",]$mean-SG_ammonium_N_trt_AC_trt[SG_ammonium_N_trt_AC_trt$precip=="A",]$mean)/SG_ammonium_N_trt_AC_trt[SG_ammonium_N_trt_AC_trt$precip=="A",]$mean)*100)
...
df_anova_SG_soil_VWC_model=data.frame(row.names = c("Precipitation","Microbial treatment","Life-stage","Precip.*Mic.Treat.","Precip.*Life-stage","Mic.Treat.*Life-stage","Precip.*Mic.Treat.*Life-stage"),"Df"=anova_SG_soil_VWC_model$NumDF,"Resid Df"=anova_SG_soil_VWC_model$DenDF,"F-value"=anova_SG_soil_VWC_model$`F value`,"P-value"=anova_SG_soil_VWC_model$`Pr(>F)`)
...
soil_grav_moist_out_trt=summarise_at(combin_grav_moist_seed_trt_soil_root_precip_g, 
                                     "percent_soil_moisture_dry_weight", list(~n(),~mean(.),~sd(.),se=~sd(.)/sqrt(n())))
...
paste("Percentage difference =",((SG_inorg_N_trt_AC_trt[SG_inorg_N_trt_AC_trt$precip=="D",]$mean-SG_inorg_N_trt_AC_trt[SG_inorg_N_trt_AC_trt$precip=="A",]$mean)/SG_inorg_N_trt_AC_trt[SG_inorg_N_trt_AC_trt$precip=="A",]$mean)*100)
...
### Change in soil nitrate from start to end

```{r}
#Change in NO3
paste("Soil NO3 increase =",(mean(SG_inorg_N_trt$ug_N_NO3_g_dry_soil)-mean(SG_start_inorg_N$ug_N_NO3_g_dry_soil))/mean(SG_start_inorg_N$ug_N_NO3_g_dry_soil))

#Change in NH4
paste("Soil NH4 increase =",(mean(SG_inorg_N_trt$ug_N_NH4_g_dry_soil)-mean(SG_start_inorg_N$ug_N_NH4_g_dry_soil))/mean(SG_start_inorg_N$ug_N_NH4_g_dry_soil))
...
df_Anova_SG_Soil_nit_start=data.frame(row.names =c("(Intercept)","Microbial treatment", "Block","Residuals"),"Df"=Anova_SG_Soil_nit_start$Df,"F-value"=Anova_SG_Soil_nit_start$`F value`,"P-value"=Anova_SG_Soil_nit_start$`Pr(>F)`)
...
df_Anova_SG_Soil_nit_start_pres=data.frame(row.names =c("(Intercept)","Microbial Presence", "Block","Residuals"),"Df"=Anova_SG_Soil_nit_start_pres$Df,"F-value"=Anova_SG_Soil_nit_start_pres$`F value`,"P-value"=Anova_SG_Soil_nit_start_pres$`Pr(>F)`)
kable(df_Anova_SG_Soil_nit_start_pres,align = "c",digits = c(2,2,3), caption= "Soil Nitrate")
```

### Microhabitat Start Soil nitrate####
...
df_Anova_SG_Soil_nit_start_orig=data.frame(row.names =c("(Intercept)","Microbial Source", "Block","Residuals"),"Df"=Anova_SG_Soil_nit_start_orig$Df,"F-value"=Anova_SG_Soil_nit_start_orig$`F value`,"P-value"=Anova_SG_Soil_nit_start_orig$`Pr(>F)`)
...
df_Anova_SG_Soil_amm_start=data.frame(row.names =c("(Intercept)","Microbial treatment", "Block","Residuals"),"Df"=Anova_SG_Soil_amm_start$Df,"F-value"=Anova_SG_Soil_amm_start$`F value`,"P-value"=Anova_SG_Soil_amm_start$`Pr(>F)`)
...
df_Anova_SG_Soil_amm_start_pres=data.frame(row.names =c("(Intercept)","Microbial Presence", "Block","Residuals"),"Df"=Anova_SG_Soil_amm_start_pres$Df,"F-value"=Anova_SG_Soil_amm_start_pres$`F value`,"P-value"=Anova_SG_Soil_amm_start_pres$`Pr(>F)`)
kable(df_Anova_SG_Soil_amm_start_pres,align = "c",digits = c(2,2,3), caption= "Soil Ammonium")
```

### Presence Start Soil Ammonium Percentage difference 


```{r echo=FALSE}

paste("Percentage difference =",((soil_amm_START_trt[soil_amm_START_trt$soil_root=="S.B",]$mean-soil_amm_START_trt[soil_amm_START_trt$soil_root=="L.B",]$mean)/soil_amm_START_trt[soil_amm_START_trt$soil_root=="L.B",]$mean)*100)
#568.581806566453
...
### Microhabitat Start Soil Ammonium####
...
df_Anova_SG_Soil_amm_start_orig=data.frame(row.names =c("(Intercept)","Microbial Source", "Block","Residuals"),"Df"=Anova_SG_Soil_amm_start_orig$Df,"F-value"=Anova_SG_Soil_amm_start_orig$`F value`,"P-value"=Anova_SG_Soil_amm_start_orig$`Pr(>F)`)
...
head(SG_trt)
SG_MBC_DOC <- read.csv(here::here("R_data","SG_analyzed_MBC_DOC.csv"))
head(SG_MBC_DOC)
...
df_Anova_SG_Soil_DOC_mod=data.frame(row.names =c("(Intercept)","Microbial treatment", "Block","Residuals"),"Df"=Anova_SG_Soil_DOC_mod$Df,"F-value"=Anova_SG_Soil_DOC_mod$`F value`,"P-value"=Anova_SG_Soil_DOC_mod$`Pr(>F)`)
...
(soil_DOC_trt_p=ggplot(soil_DOC_trt_SUM, aes(x=factor(soil_root,levels = treatment_order),fill=factor(soil_root,levels = treatment_order)))+
    ...
    scale_x_discrete(labels=c("Sterile","Bulk","Rhizo"), name=NULL)+ylab("Dissolved organic carbon (µg/g dry soil)")+
...
df_Anova_SG_Soil_MBC_mod=data.frame(row.names =c("(Intercept)","Microbial treatment", "Block","Residuals"),"Df"=Anova_SG_Soil_MBC_mod$Df,"F-value"=Anova_SG_Soil_MBC_mod$`F value`,"P-value"=Anova_SG_Soil_MBC_mod$`Pr(>F)`)
...
(soil_DOC_trt_p=ggplot(soil_MBC_trt_SUM, aes(x=factor(soil_root,levels = treatment_order),fill=factor(soil_root,levels = treatment_order)))+
    ...
    geom_errorbar(aes(y=mean, ymin = mean-se, ymax= mean+se),position=position_dodge(width=0.9), width=0.2, size=1)+
    scale_fill_manual(values = c( "white","light gray", "dark grey"),labels=c("Sterile","Bulk","Rhizo"), limits=treatment_order, name=NULL)+
    scale_x_discrete(labels=c("Sterile","Bulk","Rhizo"), name=NULL)+ylab("Microbial biomass carbon (µg/g dry soil)")+
...
Anova_SG_combin_surv_model=data.frame(row.names =c("(Intercept)","Precipitation","Microbial treatment","Life-stage","Precip.*Mic.Treat.","Precip.*Life-stage","Mic.Treat.*Life-stage","Precip.*Mic.Treat.*Life-stage"),"Df"=Anova_SG_combin_surv_model$Df,"Chi-Sq"=Anova_SG_combin_surv_model$Chisq,"P-value"=Anova_SG_combin_surv_model$`Pr(>Chisq)`)
...
Anova_SG_combin_surv_drought_model=data.frame(row.names =c("(Intercept)","Microbial treatment","Life-stage","Mic.Treat.*Life-stage"),"Df"=Anova_SG_combin_surv_drought_model$Df,"Chi-Sq"=Anova_SG_combin_surv_drought_model$Chisq,"P-value"=Anova_SG_combin_surv_drought_model$`Pr(>Chisq)`)
...
Anova_SG_combin_surv_drought_pres_model=data.frame(row.names =c("(Intercept)","Microbial Presence","Life-stage","Mic.Pres.*Life-stage"),"Df"=Anova_SG_combin_surv_drought_pres_model$Df,"Chi-Sq"=Anova_SG_combin_surv_drought_pres_model$Chisq,"P-value"=Anova_SG_combin_surv_drought_pres_model$`Pr(>Chisq)`)
kable(Anova_SG_combin_surv_drought_pres_model,align = "c",digits = c(2,2,3), caption= "Survival")
```


### Microhabitat combined survival
Diagnostic Graphs
```{r}
...
Anova_SG_combin_surv_drought_orig_model=data.frame(row.names =c("(Intercept)","Microbial Source","Life-stage","Mic.Source*Life-stage"),"Df"=Anova_SG_combin_surv_drought_orig_model$Df,"Chi-Sq"=Anova_SG_combin_surv_drought_orig_model$Chisq,"P-value"=Anova_SG_combin_surv_drought_orig_model$`Pr(>Chisq)`)
...
df_anova_SG_combin_biomass_model=data.frame(row.names = c("Microbial treatment","Precipitation","Life-stage","Precip.*Mic.Treat.","Mic.Treat.*Life-stage","Precip.*Life-stage","Precip.* Mic.Treat. *Life-stage"),"Df"=anova_SG_combin_biomass_model$NumDF,"Resid Df"=anova_SG_combin_biomass_model$DenDF,"F-value"=anova_SG_combin_biomass_model$`F value`,"P-value"=anova_SG_combin_biomass_model$`Pr(>F)`)
...
df_anova_SG_combin_biomass_pres_model=data.frame(row.names = c("Microbial Presence","Precipitation","Life-stage","Precip.*Mic.Pres.","Mic.Pres.*Life-stage","Precip.*Life-stage","Precip.* Mic.Pres. *Life-stage"),"Df"=anova_SG_combin_biomass_pres_model$NumDF,"Resid Df"=anova_SG_combin_biomass_pres_model$DenDF,"F-value"=anova_SG_combin_biomass_pres_model$`F value`,"P-value"=anova_SG_combin_biomass_pres_model$`Pr(>F)`)
kable(df_anova_SG_combin_biomass_pres_model,align = "c",digits = c(1,2,2,3), caption= "Total biomass")
```


#### Percent Difference sterile v live
```{r echo=FALSE}

data_SG_biomass_trt_comb_soil_root_g=data_SG_biomass_trt_comb %>% group_by(soil_root)
comb_total_bio_soil_root=summarise_at(data_SG_biomass_trt_comb_soil_root_g, 
                                        "total_biomass", list(~n(),~mean(.),~sd(.),se=~sd(.)/sqrt(n())))


paste("Percentage difference =",((comb_total_bio_soil_root[comb_total_bio_soil_root$soil_root=="S.B",]$mean-comb_total_bio_soil_root[comb_total_bio_soil_root$soil_root=="L.B",]$mean)/comb_total_bio_soil_root[comb_total_bio_soil_root$soil_root=="L.B",]$mean)*100)

```


### Microhabitat Combined Biomass
...
df_anova_SG_combin_biomass_orig_model=data.frame(row.names = c("Microbial Source","Precipitation","Life-stage","Precip.*Mic.Source","Mic.Source*Life-stage","Precip.*Life-stage","Precip.* Mic.Source *Life-stage"),"Df"=anova_SG_combin_biomass_orig_model$NumDF,"Resid Df"=anova_SG_combin_biomass_orig_model$DenDF,"F-value"=anova_SG_combin_biomass_orig_model$`F value`,"P-value"=anova_SG_combin_biomass_orig_model$`Pr(>F)`)
...
df_anova_SG_combin_root_shoot_model=data.frame(row.names = c("Microbial treatment","Precipitation","Life-stage","Precip.*Mic.Treat.","Mic.Treat.*Life-stage","Precip.*Life-stage","Precip.* Mic.Treat. *Life-stage"),"Df"=anova_SG_combin_root_shoot_model$NumDF,"Resid Df"=anova_SG_combin_root_shoot_model$DenDF,"F-value"=anova_SG_combin_root_shoot_model$`F value`,"P-value"=anova_SG_combin_root_shoot_model$`Pr(>F)`)
...
df_anova_SG_combin_root_shoot_pres_model=data.frame(row.names = c("Microbial Presence","Precipitation","Life-stage","Precip.*Mic.Pres.","Mic.Pres.*Life-stage","Precip.*Life-stage","Precip.* Mic.Pres. *Life-stage"),"Df"=anova_SG_combin_root_shoot_pres_model$NumDF,"Resid Df"=anova_SG_combin_root_shoot_pres_model$DenDF,"F-value"=anova_SG_combin_root_shoot_pres_model$`F value`,"P-value"=anova_SG_combin_root_shoot_pres_model$`Pr(>F)`)
kable(df_anova_SG_combin_root_shoot_pres_model,align = "c",digits = c(1,2,2,3), caption= "Total biomass")
```

### Microhabitat Combined Root:Shoot
...
df_anova_SG_combin_root_shoot_orig_model=data.frame(row.names = c("Microbial Source","Precipitation","Life-stage","Precip.*Mic.Source","Mic.Source*Life-stage","Precip.*Life-stage","Precip.* Mic.Source *Life-stage"),"Df"=anova_SG_combin_root_shoot_orig_model$NumDF,"Resid Df"=anova_SG_combin_root_shoot_orig_model$DenDF,"F-value"=anova_SG_combin_root_shoot_orig_model$`F value`,"P-value"=anova_SG_combin_root_shoot_orig_model$`Pr(>F)`)
